...
from .quant_layer import QuantModule, lp_loss, QuantMatMul
from .quant_model import QuantModel
from .quant_block import BaseQuantBlock, specials_unquantized, QuantAttention, QuantWindowAttention
...
def block_reconstruction(model: QuantModel, fp_model: QuantModel, block: BaseQuantBlock, fp_block: BaseQuantBlock,
    ...
    for module in block.modules():
        ...
        #         module.quantizer_A.delta = torch.nn.Parameter(torch.tensor(module.quantizer_A.delta))
        #         a_para += [module.quantizer_A.delta]
        #     if module.quantizer_B.delta is not None:
        #         module.quantizer_B.delta = torch.nn.Parameter(torch.tensor(module.quantizer_B.delta))
        #         a_para += [module.quantizer_B.delta]
        #     '''set up drop'''
        #     module.quantizer_A.is_training = True
        #     module.quantizer_B.is_training = True
            
        # if isinstance(module, (QuantModule, BaseQuantBlock)):
        if isinstance(module, QuantModule):
            # if module.outlier_flag != True:
            ...
            #     # if module.outlier_flag == True:
            #     #     module.act_quantizer.outlier_alpha = torch.nn.Parameter(torch.tensor(module.act_quantizer.outlier_alpha))
            #     #     a_para += [module.act_quantizer.outlier_alpha]
            #     '''set up drop'''
    ...
    for i in range(iters):
        ...
        if isinstance(block, (QuantAttention, QuantWindowAttention)):
            cur_out = cached_outs[0][idx].to(device)
            cur_atten_score = cached_outs[1][idx].to(device)
        else:
            cur_out = cached_outs[idx].to(device)  #fp layer out
        if input_prob < 1.0:
        ...
        if isinstance(block, (QuantAttention, QuantWindowAttention)):
            out_drop = out_all[0][:batch_size]   # quantization layer out
            out_atten_score = out_all[1][:batch_size]
        else:
            out_drop = out_all[:batch_size]   # quantization layer out
        # print(f"quantization layer out shape:{out_drop.shape},{out_all.shape},{cur_out.shape}")
        out_fp_layer = cur_out
        out_quant = out_all[batch_size:]
        output = out_quant
        # output_fp_layer = out_fp_layer
        # if isinstance(block, QuantAttention):
        #     for num, module in enumerate(module_list):
        #         # for ResNet and RegNet
        #         # if name_list[num] == 'fc':
        #         #     output = torch.flatten(output, 1)
        #         if name_list[num] == "head":
        #             output = output[:, 0]
        #         output = module(output)
        #         output_fp_layer = module(output_fp_layer)
        #         if name_list[num] == "norm2":
        #             break
        #     err = loss_func(out_drop, cur_out, output, output_fp_layer, pd_loss = True)
        
        if isinstance(block, (QuantAttention, QuantWindowAttention)):
            err = loss_func(out_drop, cur_out, cur_atten_score, out_atten_score, pd_loss = True)
        else: 
            err = loss_func(out_drop, cur_out, pd_loss = False)
...
def set_act_quant_block(model: QuantModel, block: BaseQuantBlock, 
                        cali_data: torch.Tensor, batch_size: int = 32, iters: int = 20000, weight: float = 0.01, 
                        opt_mode: str = 'mse', b_range: tuple = (20, 2),
                        warmup: float = 0.0, p: float = 2.0, lr: float = 4e-5,
                        input_prob: float = 1.0, keep_gpu: bool = True, 
...
class LossFunction:
    ...
    def __call__(self, pred, tgt, output=None, output_fp=None, pd_loss = False):
    # def __call__(self, pred, tgt):
        ...
        if pd_loss: 
            pd_loss = self.pd_loss(F.log_softmax(output / self.T, dim=1), F.softmax(output_fp / self.T, dim=1)) / self.lam
            # pd_loss = lp_loss(output, output_fp, p=self.p)
