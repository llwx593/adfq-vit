...
from .quant_block import BaseQuantBlock, QuantAttention, QuantWindowAttention
...
def save_dc_fp_data(model: QuantModel, layer: Union[QuantModule, BaseQuantBlock], cali_data: torch.Tensor,
    ...
    for i in trange(int(cali_data.size(0) / batch_size)):
        if input_prob:
            ...
            if isinstance(layer, (QuantAttention, QuantWindowAttention)):
                cached_batches.append(((cur_out[0].cpu(),cur_out[1].cpu()), out_fp.cpu(), cur_sym.cpu()))
            else:
    ...
    if isinstance(layer, (QuantAttention, QuantWindowAttention)):
        cached_outs = (torch.cat([x[0][0] for x in cached_batches]), torch.cat([x[0][1] for x in cached_batches]))
    else:
    ...
    if keep_gpu:
        if isinstance(layer, (QuantAttention,QuantWindowAttention)):
            cached_outs = (cached_outs[0].to(device),cached_outs[1].to(device))
        else:
    ...
    if input_prob:
        if isinstance(layer, (QuantAttention, QuantWindowAttention)):
            cached_outs[0].requires_grad = False
            cached_outs[1].requires_grad = False
        else:
...
class GetDcFpLayerInpOut:
    ...
    def __call__(self, model_input):
        ...
        if self.input_prob:
            if isinstance(self.layer, (QuantAttention, QuantWindowAttention)):
                return  (out_fp[0].detach(), out_fp[1].detach()), output_fp.detach(), para_input.detach()
            else:
